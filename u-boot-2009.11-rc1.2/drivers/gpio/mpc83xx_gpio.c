...
{
	immap_t *im = (immap_t *)CONFIG_SYS_IMMR;
//	volatile gpio83xx_t *gpio_im = &((immap_t *)(CONFIG_SYS_IMMR))->gpio[0];
	unsigned int ctrlr;
	unsigned int line;
	unsigned int line_mask;
	...
	out_be32(&im->gpio[0].dir, line_mask);

//	out_be32(&im->sysconf.sicrh, syscfg);
//printf("dir=0x%x\n",in_be32(&im->gpio[ctrlr].dir));
...
int gpio_set_value(unsigned gpio, int value)
{
	...
	/* Write the output */
	out_be32(&im->gpio[0].dat, gpio_output_value[0]);
