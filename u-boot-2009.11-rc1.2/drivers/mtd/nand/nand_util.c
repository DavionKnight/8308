			...
			u_char *buffer)
{
...
#if defined(CONFIG_MTD_NAND_YAFFS2)        /* Support YAFFS2 : Yangyf */
	if (nand->rw_oob == 1) {
	...
	if (nand->skipfirstblk == 1)
		len_incl_bad += nand->erasesize - (offset & (nand->erasesize - 1));
#endif
...
#if defined(CONFIG_MTD_NAND_YAFFS2)        /* Support YAFFS2 : Yangyf */
		if (nand->skipfirstblk == 1) {    /* 需要跳过第一块 */
			nand->skipfirstblk = 0;
			printf("Skip the first good block 0x%08llx\n",offset &    
			~(nand->erasesize - 1));
			offset += nand->erasesize - block_offset;/* 故偏移地址 + 1block */
			continue;
		}
#endif		
