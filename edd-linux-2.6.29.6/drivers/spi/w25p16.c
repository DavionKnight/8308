 ...
 */
int w25p16_erase_chip(struct w25p *flash)
{
 ...
 * Returns 0 if successful, non-zero otherwise.
 */
int w25p16_erase_sector(struct w25p *flash, u32 offset)
{
...
static int w25p16_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	/* whole-chip erase? */
	if (len == flash->mtd.size && w25p16_erase_chip(flash)) {
	...
	/* "sector"-at-a-time erase */
		...
		while (len) {
			if (w25p16_erase_sector(flash, addr)) {
				instr->state = MTD_ERASE_FAILED;
				mutex_unlock(&flash->lock);
				return -EIO;
			}
