...
static void esdhc_prepare_data(struct esdhc_host *host, struct mmc_data *data)
{
	 ...
	 *     (1) / (2) > 2^6
	 */
	/* For some SDHC card, the timeout must be 0xE */
	count = 0xE;
	...
	struct mmc_data *data)
{
	...
	if (data->blocks > 1)
		...
		mode |= ESDHC_TRNS_ACMD12; /* remove when ACMD12 bug is fixed */
	if (data->flags & MMC_DATA_READ)
...
static void esdhc_finish_data(struct esdhc_host *host)
{
	...
	if ((data->blocks == 1) && (data->error == 0))
		...
		/* uncomment line when ACMD12 bug is fixed
		blocks = fsl_readl(host->ioaddr + ESDHC_BLOCK_ATTR) >> 16; */
		blocks = 0;	/* remove when ACMD12 bug is fixed */
...
static void esdhc_set_clock(struct esdhc_host *host, unsigned int clock)
{
	...
	if (host->card_flag)
		setbits32(host->ioaddr + ESDHC_SYSTEM_CONTROL,
			  ESDHC_CLOCK_CARD_EN | ESDHC_SD_CARD_CLOCK_EN);
	else
		setbits32(host->ioaddr + ESDHC_SYSTEM_CONTROL,
			  ESDHC_CLOCK_CARD_EN);
	esdhc_dumpregs(host);
...
static int esdhc_get_ro(struct mmc_host *mmc)
{
	...
	/* For MPC8308, WPSPL=0 for Write protect */
	if (host->card_flag)
...
static int __devinit esdhc_probe_slot(struct of_device *ofdev, int slot)
{
	...
	host->card_flag = 0;
	if (of_device_is_compatible(np, "fsl,mpc8308-esdhc"))
		host->card_flag = 1;
