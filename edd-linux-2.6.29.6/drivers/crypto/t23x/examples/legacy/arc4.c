...
#define RC4_SHORT_KEYSIZE  (8)
#define RC4_SHORT_TESTSIZE (40)
#define RC4_SHORT_CTXSIZE  (259)

static const unsigned char AFKey[] =
{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07};

static unsigned char rc4TestData[RC4_SHORT_TESTSIZE];
static unsigned char rc4EncResult[RC4_SHORT_TESTSIZE];
static unsigned char rc4Result[RC4_SHORT_TESTSIZE];
static unsigned char rc4Ctx[RC4_SHORT_CTXSIZE];


int arc4short(int fd)
{
    int status, i, teststat;
    ARC4_LOADKEY_CRYPT_UNLOADCTX_REQ afReq;

    memset(&afReq,       0, sizeof(afReq));
    memset(rc4EncResult, 0, RC4_SHORT_TESTSIZE);
    memset(rc4Result,    0, RC4_SHORT_TESTSIZE);

    /* precoat test array */
    for (i = 0; i < RC4_SHORT_TESTSIZE; i++)
        rc4TestData[i] = 0x03;


    /* encryption (first) pass */
    afReq.opId            = DPD_RC4_LDKEY_CRYPT_ULCTX;
    afReq.keyBytes        = RC4_SHORT_KEYSIZE;
    afReq.keyData         = (unsigned char *)AFKey;
    afReq.inBytes         = RC4_SHORT_TESTSIZE;
    afReq.inData          = rc4TestData;
    afReq.outData         = rc4EncResult;
    afReq.outCtxBytes     = RC4_SHORT_CTXSIZE;
    afReq.outCtxData      = rc4Ctx;

    status = ioctl(fd, IOCTL_PROC_REQ_BLOCK_VIRTUAL, (int)&afReq);
    ...
    if (status)
    {
        ...
        {
            consolemsg("arc4", "arc4short", "", "not supported");
            return 0;
        }
        else
        {
            printf("arc4: arc4short - driver error with 0x%08x\n", status);
            return -1;
        }
    }
    ...
    /* compare */
    if(memcmp(rc4TestData, rc4Result, RC4_SHORT_TESTSIZE) == 0) {
        consolemsg("arc4", "arc4short", "", "OK");
        teststat = 0;
    } else {
        consolemsg("arc4", "arc4short", "", "error");
        printf("received:\n");
        dumpm(rc4Result, RC4_SHORT_TESTSIZE);
        printf("\nshould be:\n");
        dumpm(rc4TestData, RC4_SHORT_TESTSIZE);
        printf("\n");
        teststat = -1;
    }
...
#define RC4_CTX_TESTSIZE (2048)
#define RC4_CTX_CTXSIZE  (259)

static unsigned char cmpdata[RC4_CTX_TESTSIZE], cmpctx[RC4_CTX_CTXSIZE];
...
int arc4ctx(int fd)
{
    ...
    rc4RQ.inCtxBytes      = RC4_CTX_CTXSIZE;
    rc4RQ.inCtxData       = (unsigned char *)inctx;
    rc4RQ.inBytes         = RC4_CTX_TESTSIZE;
    rc4RQ.inData          = (unsigned char *)indata;
    rc4RQ.outData         = cmpdata;
    rc4RQ.outCtxBytes     = RC4_CTX_CTXSIZE;
    rc4RQ.outCtxData      = cmpctx;

    status = ioctl(fd, IOCTL_PROC_REQ_BLOCK_VIRTUAL, (int)&rc4RQ);
    ...
    if (status)
    {
        if ((status == SEC2_INVALID_CHA_TYPE) ||
            (status == SEC2_CHA_ERROR))
        {
            consolemsg("arc4", "arc4ctx", "", "not supported");
            return 0;
        }
        else
        {
            printf("arc4: arc4ctx - driver error with 0x%08x\n", status);
            return -1;
        }
    }

    /* compare data */
    if (memcmp(cmpdata, outdata, RC4_CTX_TESTSIZE))
        teststat = -1;

    if (memcmp(cmpctx, outctx, RC4_CTX_CTXSIZE))
        teststat = -1;

    if (!teststat)
        consolemsg("arc4", "arc4ctx", "", "OK");
    else
        consolemsg("arc4", "arc4ctx", "", "error");
...
#define RC4_LONG_KEYSIZE  (10)
#define RC4_LONG_CTXSIZE  (259)
#define RC4_LONG_TESTSIZE (16384)


static const unsigned char AFlongKey[] =
{
...
static unsigned char rc4longCtx[RC4_LONG_CTXSIZE];
static unsigned char rc4longResult[RC4_LONG_TESTSIZE];
...
int arc4long(int fd)
{
    ...
    memset(&afReq,    0, sizeof(afReq));
    memset(rc4Ctx,    0, RC4_LONG_CTXSIZE);
    memset(rc4Result, 0, RC4_LONG_TESTSIZE);

    afReq.opId            = DPD_RC4_LDKEY_CRYPT_ULCTX;
    afReq.keyBytes        = RC4_LONG_KEYSIZE;
    afReq.keyData         = (unsigned char *)AFlongKey;
    afReq.inBytes         = RC4_LONG_TESTSIZE;
    afReq.inData          = (unsigned char *)test_data;
    afReq.outData         = rc4longResult;
    afReq.outCtxBytes     = RC4_LONG_CTXSIZE;
    afReq.outCtxData      = rc4longCtx;
    ...
    if (status)
    {
        if ((status == SEC2_INVALID_CHA_TYPE) ||
            (status == SEC2_CHA_ERROR))
        {
            consolemsg("arc4", "arc4long", "", "not supported");
            return 0;
        }
        else
        {
            printf("arc4: arc4long - driver error with 0x%08x\n", status);
            return -1;
        }
    }
    ...
    /* compare */
    if(memcmp(expected_data, rc4longResult, RC4_LONG_TESTSIZE) == 0) {
        consolemsg("arc4", "arc4long", "", "OK");
        status = 0;
    } else {
        consolemsg("arc4", "arc4long", "", "error");
        status = -1;
    }
