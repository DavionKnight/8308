...
int f8(int fd)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        status = ioctl(fd, IOCTL_PROC_REQ_BLOCK_VIRTUAL, (int)&keaRq);
        ...
        if (status)
        {
            ...
            {
                consolemsg("kasumi", "f8", "", "not supported");
                return 0;
            }
            else
            {
                printf("kasumi: f8 - driver error with 0x%08x\n", status);
                return -1;
            }
        }
        ...
        if ((memcmp(f8_testdata[i].f8_expout, encryptBuf, keaRq.outBytes)) == 0)
            consolemsg("kasumi", "f8", setname, "OK");
        else
        {
            consolemsg("kasumi", "f8", setname, "error");
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (status)
        {
            if ((status == SEC2_INVALID_CHA_TYPE) ||
                (status == SEC2_CHA_ERROR))
            {
                consolemsg("kasumi", "f9", "", "not supported");
                return 0;
            }
            else
            {
                printf("kasumi: f9 - driver error with 0x%08x\n", status);
                return -1;
            }
        }
        ...
        if ((memcmp(f9_testdata[i].f9_expout, ctxBuf, keaRq.ctxBytes)) == 0)
            consolemsg("kasumi", "f9", setname, "OK");
        else
        {
            consolemsg("kasumi", "f9", setname, "error");
            printf("expected result:\n");
