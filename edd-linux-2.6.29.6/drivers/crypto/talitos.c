...
struct talitos_private {
	...
	/* XOR Device */
	struct fsl_xor_adma_device *xor_dev;
};
...
static int talitos_remove(struct of_device *ofdev)
{
	...
	if (priv->xor_dev)
		fsl_xor_adma_unregister(priv->xor_dev);
...
void talitos_release_xor(struct device *dev,
			 struct talitos_desc *desc, void *context, int error)
{
	struct fsl_xor_desc *xor_desc = context;

	kfree(desc);
	xor_desc->release_hdl(xor_desc->release_arg, error);
}
			...
			void *context)
{
	...
	desc->ptr[TOTAL_PAIRS - 1].ptr = cpu_to_be32(dest);
	desc->ptr[TOTAL_PAIRS - 1].len = cpu_to_be16(len);
	desc->ptr[TOTAL_PAIRS - 1].j_extent = 0;
	desc->ptr[TOTAL_PAIRS - 1].eptr = 0;

	/* Set Sources: End loading from second-last pointer pair */
	for (i = (TOTAL_PAIRS - 2), j = 0;
	     (j < count) && (i > 0); i--, j++) {
		desc->ptr[i].ptr = cpu_to_be32(src_list[j]);
	...
	desc->hdr = DESC_HDR_SEL0_AESU | DESC_HDR_MODE0_AESU_XOR
	    | DESC_HDR_TYPE_RAID_XOR | DESC_HDR_DIR_OUTBOUND;

	ret = talitos_submit(dev, desc, talitos_release_xor, context);
			 ...
			 const struct of_device_id *match)
{
	...
	if (of_device_is_compatible(np, "fsl,sec3.0"))
		max_xor_srcs = TOTAL_PAIRS - 1;	/* Sec3.x and above have support
						   upto 6 RAID Sources */
	else if (hw_supports(dev, DESC_HDR_TYPE_RAID_XOR))
		max_xor_srcs = TOTAL_PAIRS - 4;	/* Sec 2.1 and above support
						   upto 3 RAID sources */

	if (max_xor_srcs) {
		if (fsl_xor_adma_register(&priv->xor_dev, max_xor_srcs, dev)) {
			dev_err(dev, "XOR not registered\n");
			priv->xor_dev = NULL;
		}
	}
